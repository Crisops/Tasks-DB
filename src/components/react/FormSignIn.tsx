...
import { handleSubmit } from '../../helpers/fetchSignIn'
  ...
  return (
    <div className='absolute top-0 left-0 w-1/2 h-full flex justify-center'>
      <form onSubmit={SignIn} className='w-96 h-full flex flex-col justify-center'>
        <input onChange={() => selectProvider} hidden name='provider' value={provider} />
        <div className='flex flex-col justify-center items-center gap-y-4'>
          <h1 className='text-4xl font-bold text-center'>Sign in</h1>
          <div className='flex gap-x-6'>
            <ButtonProviderLogin signInProvider={selectProvider} provider='github' ProviderLogin={GitHub} />
            <ButtonProviderLogin signInProvider={selectProvider} provider='google' ProviderLogin={Google} />
            <ButtonProviderLogin signInProvider={selectProvider} provider='facebook' ProviderLogin={Facebook} />
          </div>
          <div>
